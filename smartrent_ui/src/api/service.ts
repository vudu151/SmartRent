...
export const getFeeConfig = async () => {
  ...
  if (!buildingId) return {}; // Tránh gọi lỗi nếu chưa có building
  
  const res = await apiFetch<ApiResponse<any>>(`/api/services/buildings?tenantId=${tenantId}&buildingId=${buildingId}`);
...
export const updateFeeConfig = async (data: any) => {
  const tenantId = getTenantId();
  const buildingId = localStorage.getItem("selectedBuildingId");
  if (!buildingId) throw new Error("Chưa chọn Khu trọ");
...
export const recordMeterReading = async (data: any) => {
  const tenantId = getTenantId();
  const res = await apiFetch<ApiResponse<any>>(`/api/services/meters?tenantId=${tenantId}`, {
    method: 'POST',
    body: data,
  });
  if (!res.success) throw new Error(res.message);
...
export const generateCombinedBill = async (roomId: number, month: number, year: number) => {
  const tenantId = getTenantId();
  const res = await apiFetch<ApiResponse<any>>(`/api/services/generate-bill?tenantId=${tenantId}&roomId=${roomId}&month=${month}&year=${year}`, {
    method: 'POST',
  });
  if (!res.success) throw new Error(res.message);
  return res.data;
